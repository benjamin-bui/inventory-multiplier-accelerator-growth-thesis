...
\begin{figure}[ht]
    \centering
    \includegraphics[height=0.4\textheight]{./metzlerian_growth/timeseries3.eps}
    \caption{Timeseries plot of income growth rate over 150 iterations. Parameters are identical to the model presented in Figure \ref{growth_timeseries1}. Initial conditions of $\dot Y$ are: 29. 20, 15, 10, 20, 50}
    \label{growth_timeseries3}
\end{figure}
Figure \ref{growth_timeseries3} displays a trajectory of income growth given identical parameters to the timeseries displayed in \ref{growth_timeseries1}. However, the dynamics of the model are qualitatively different, indicating long-run sensitivity to initial conditions. 

\begin{figure}
    \centering
    \includegraphics[height=0.4\textheight]{./metzlerian_growth/sbifurcation.eps}
...
\begin{figure}
    \centering
    \includegraphics[height=0.4\textheight]{./metzlerian_growth/timeseries2.eps}
...
$k$ directly affects inventory production levels by shifting the desired level of inventory relative to predicted production. Although the value of the parameter only makes intuitive sense when bounded between 0 and 1, it would realistically not approach either extreme. If $k$ is too low, firms would not be able to handle unexpected demand shocks and $k$ would only approach 1 if it determined that its predictive mechanism was frequently incorrect in which case it is more likely that firms would develop a more accurate mechanism than waste resources creating large amounts of inventory. It is possible for $k>1$; however, these scenarios are not considered within the scope of this paper.

\begin{figure}
    \centering
    \includegraphics[height=0.4\textheight]{./metzlerian_growth/kbifurcation.eps}
